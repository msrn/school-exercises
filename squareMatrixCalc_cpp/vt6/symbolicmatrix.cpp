...
SymbolicSquareMatrix SymbolicSquareMatrix::operator+(const SymbolicSquareMatrix& m) const {
    ...
    SymbolicSquareMatrix copy = m;
    //n = m.n;
    SymbolicSquareMatrix matrix{m};

    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; i++) {
            CompositeElement elem{*elements[i][j], *copy.elements[i][j], std::plus<int>{}, '+'};
            matrix.elements[i][j] = std::unique_ptr<Element>{elem.clone()};
        }
    }
    return matrix;
}
...
SymbolicSquareMatrix SymbolicSquareMatrix::operator-(const SymbolicSquareMatrix& m) const {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; i++) {
            CompositeElement elem{*elements[i][j], *copy.elements[i][j], std::minus<int>{}, '-'};
            matrix.elements[i][j] = std::unique_ptr<Element>{elem.clone()};
        }
    }
    return matrix;
